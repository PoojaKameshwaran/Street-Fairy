...
def screen_2():
   ...
   ##########Added by Deepana
    ...
    if user_message:
        ...
        ########Added by Deepana
        if user_feedback := st.text_input("Provide your feedback"):
            st.session_state.chat_history.append({"role": "user", "content": user_feedback})
            # Process the feedback (e.g., re-prompt with better suggestions)
            if "not for me" in user_feedback.lower():
                # If the user is not happy with the suggestion, offer a new one
                if st.session_state.remaining_recs:
                    next_suggestion = st.session_state.remaining_recs.pop(0)
                    next_prompt = f"""
                    You are Street Fairy 🧚‍♀️. The last place wasn't quite right.
                ...
                # Continue the conversation, ask for more details if needed
